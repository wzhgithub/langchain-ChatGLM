...
def qa_main(file_index_path, submit_file, device, suffix):
    # llm_model_ins = None
    ...
    with jsonlines.open(f"{submit_file}_submit_{suffix}.json", mode='w') as sjl:
        # lines = []
        with jsonlines.open(submit_file, 'r') as jl:
            for data in jl:
                ...
                # lines.append(data)
                try:
                    index_vs_paths = get_index_list(local_doc_qa, name_set, query)
...
def get_index_list(local_doc_qa, name_set, query):
    from pypinyin import lazy_pinyin
    vs_paths = get_name_set_path(name_set, query)
    logger.info(f"from name set get vs_path:{vs_paths}")
    if len(vs_paths) == 0:
        logger.info(f"query:{query} not find in company pdf")
        # return ['./pdf/index/']
        return []
    index_vs_paths = []
...
def main(gpus, index, device_num, debug):
    ...
    local_doc_qa.init_knowledge_vector_store(pdf_files, "./pdf/index")
    return
    vs_path = None
    while not vs_path:
